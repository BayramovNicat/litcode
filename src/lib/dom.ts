import type { MountHandle, RepeatResult, TemplateResult, View } from './types';
...
function normalizeRepeat(result: RepeatResult): Node[] {
  const nodes: Node[] = [];
...
function updateRepeatChildPart(part: ChildPart, value: RepeatResult): void {
  const parent = part.marker.parentNode;
  if (!parent) return;
...
function updateChildPart(part: ChildPart, value: unknown): void {
  if (isRepeatResult(value)) {
    updateRepeatChildPart(part, value);
    return;
  }
