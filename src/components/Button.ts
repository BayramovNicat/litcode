import { html, component, type View, type Props, cn, tv, type VariantProps } from '@/lib';

export type RawButtonProps = Props<Partial<HTMLButtonElement>>;

export const RawButton = component<RawButtonProps>(({ children }: RawButtonProps = {}): View => {
...
export type ButtonProps = RawButtonProps & VariantProps<typeof buttonVariants>;

export const Button = component<ButtonProps>(
  ({ variant, size, className, children }: ButtonProps = {}): View => {
    return html`<button class="${cn(buttonVariants({ variant, size }), className)}">
      ${children ?? ''}
    </button>`;
  },
);
